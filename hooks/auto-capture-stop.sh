...
reason = (
    ...
    "exposed or rendered, deprecating/replacing a prior decision, or a real ops/infra "
    "choice (enable/defer/replace a service) — you MUST record it NOW via a single "
    "`kg ingest` (do NOT ask permission; use the knowledge-graph skill's DO/DON'T rules "
    "to decide what counts). If the turn made no such decision — a code-level rename, "
    "formatting, a bug fix, or ONLY analyses/reports/recommendations with no choice "
    "acted on — record nothing and just stop. Either record and note in one line what "
    "you captured, or stop."
)
